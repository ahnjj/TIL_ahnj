...
class RolePlayingRoomConsumer(JsonWebsocketConsumer):
    ...
    def connect(self):
        ...
        if room is None:
            ...
            # self.synonym_message = room.get_synonym_message() # 추천 메세지
    ...
    def receive_json(self, content_dict, **kwargs):
        ...
        # 동의어 요청 처리(서버)
        # elif content_dict['type'] == 'request-synonym-message':
        #     synonym_message = self.gpt_query(command_query=self.synonym_message)
        #     self.send_json({
        #         "type":"synonym-message",
        #         "message":synonym_message,
        #     })
        else:
            self.send_json({
                "type": "error",
